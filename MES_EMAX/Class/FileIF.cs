...
namespace MES
{
    public class FileIF
    {
        ...
        //DataTable 형식으로 변경
        public static DataTable Excel_ToDataTable(string sFileName)
        {
            ...
            IList list = ((IListSource)Excel_UpLoad(sFileName)).GetList();
            ...
            for(int i = 0; i < props.Count; i++)
            {
                ...
                dt_Return.Columns.Add(prop.Name, prop.PropertyType);
            }
            ...
            foreach(DevExpress.DataAccess.Native.Excel.ViewRow row in list)
            {
                ...
                {
                    values[i] = props[i].GetValue(row);
                }
                dt_Return.Rows.Add(values);
            }

            return dt_Return;
        }

        //그리드에 바로 뿌리기(칼럼명이 같아야 함)
        public static void Excel_ToGridControl(GridControlEx gridControl, string sFileName)
        {
            if (string.IsNullOrWhiteSpace(sFileName))
                return;

            gridControl.DataSource = Excel_UpLoad(sFileName);
        }

        private static ExcelDataSource Excel_UpLoad(string sFileName)
        {
            try
            {
                SpreadsheetControl spreadsheet = new SpreadsheetControl();
                spreadsheet.LoadDocument(sFileName);
                string sSheetName = spreadsheet.Document.Worksheets[0].Name;

                ExcelDataSource excel = new ExcelDataSource();
                excel.FileName = sFileName;

                ExcelWorksheetSettings excelSettings = new ExcelWorksheetSettings(sSheetName);
                excel.SourceOptions = new ExcelSourceOptions(excelSettings);
                ((DevExpress.DataAccess.Excel.ExcelSourceOptions)excel.SourceOptions).SkipHiddenColumns = false;
                ((DevExpress.DataAccess.Excel.ExcelSourceOptions)excel.SourceOptions).SkipHiddenRows = false;
                excel.Fill();
